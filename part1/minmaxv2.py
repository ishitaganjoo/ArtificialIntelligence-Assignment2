...
def minmax_decision(board):
    ...
    while terminal_test(best_board) == 404:
        ...
        for s in successors(fringe.pop()):
            depth = 0
            ndepth=0
            temp,ndepth = min_value(s, -infinity, infinity,depth)
            if temp > best_value:
                best_depth = ndepth
                best_value = temp
                best_board = s
            elif temp == best_value and ndepth > best_depth:
                best_depth = ndepth
                best_value = temp
                best_board = s
...
def min_value(board,alpha,beta,depth):
   # global depth
    ...
    for s in successors(board):
        val,ndepth = max_value(s, alpha, beta,depth)
        v=min(v,val)
        if v <= alpha:
            return v
        beta=min(beta,v)
...
def max_value(board, alpha, beta,depth):
    #global depth
    if terminal_test(board) != 404: 
        return terminal_test(board),depth    
    v = -infinity
    depth += 1
    for s in successors(board):
        val,ndepth = min_value(s, alpha, beta,depth)
        v=max(v,val)
        if v >= beta:
            return v
        alpha=max(alpha,v)
